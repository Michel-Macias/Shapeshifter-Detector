...
def scan_file(filepath, report_list=None):
    ...
    Envoltura con captura de excepciones para evitar fallos en multihilo.
    """
    try:
        ...
        logger.error(f"Error procesando {os.path.basename(filepath)}: {e}")
...
def main():
    ...
    if os.path.isfile(args.path):
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as executor:
            ...
            for f in track(as_completed(futures), total=len(files_to_scan), description="Procesando archivos (Multi-hilo)..."):
                try:
                    f.result() # Forzamos la captura de cualquier error interno
                except Exception as e:
